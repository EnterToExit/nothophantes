...
async def send_audio_to_process(audio_file: UploadFile = File(...)):
    ...
    response = requests.post("http://external_service:9000/asr", files={"audio_file": audio_file.file})
    if response.status_code == 200:
        print(response.text)
        process_response = requests.post("http://mistral_server:9001/nlp", json={"text": response.text})
